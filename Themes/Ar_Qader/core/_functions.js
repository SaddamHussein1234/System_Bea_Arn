...
//Toggle Visibility
function toggleVisibility(element,name=null,value=null){
	$("[visibility-control=" + (name ? name : element.name) + "][visibility-value]").hide().each(function(){
		var values = $(this).attr("visibility-value").split(',');
		if ($(element).attr("type")=="radio" || $(element).attr("type")=="checkbox"){
			var show = ($(element).prop("checked")==true && values.indexOf((value!=null ? value : element.value))!==-1);
		} else {
			var show = (values.indexOf((value!=null ? value : element.value))!==-1);
		}
		if (show){
	...
	$("[visibility-control=" + (name ? name : element.name) + "][invisibility-value]").show();
	$("[visibility-control=" + (name ? name : element.name) + "][invisibility-value=" + (value!=null ? value : element.value) + "]").hide();
}
...
function sendParentMessage(event_key,event_value){
	parent.postMessage({key:event_key,value:event_value},"*");
}
