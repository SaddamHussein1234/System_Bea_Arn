...
public partial class CResearchers_CPVillage_PageBeneficiaryHousingStatus : System.Web.UI.Page
{
    ...
    private void FGetMostafeedByMasderAlDhal(int XID)
    {
        GVMostafeedByDakhl.Columns[11].Visible = true;
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT TOP 1000 * FROM [dbo].[RasAlEstemarah] With(NoLock) inner join Quaem on Quaem.IDItem = RasAlEstemarah.AlQaryah Where AlQaryah = @0 And HaletAlMasken = @1 And TypeMostafeed = @2 And RasAlEstemarah.IsDelete = @3 And Quaem.AlQriah <> @4 Order By AlQaryah", DLAlQriah.SelectedValue, Convert.ToString(XID), "دائم", Convert.ToString(false), "مناطق_أخرى");
        if (dt.Rows.Count > 0)
        {
            txtSearchMostafeed.Text = "قائمة بيانات المستفيدين حسب حالة المسكن ( " + DLMasderAlDkhal.SelectedItem.ToString() + " ) لقرية " + DLAlQriah.SelectedItem.ToString();
