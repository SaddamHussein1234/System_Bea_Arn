...
public partial class CResearchers_CPVillage_PageBeneficiaryAddBoys : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            for (int i = Convert.ToInt32(ClassDataAccess.GetCurrentTime().ToString("yyyy")) - 579; i >= Convert.ToInt32(ClassDataAccess.GetCurrentTime().ToString("yyyy")) - 700; i--)
            {
                dtYearH.Rows.Add(i);
            }
            ...
            for (int i = Convert.ToInt32(ClassDataAccess.GetCurrentTime().ToString("yyyy")) - 579; i >= Convert.ToInt32(ClassDataAccess.GetCurrentTime().ToString("yyyy")) - 619; i--)
            {
    ...
    private void FGetDataboy()
    {
        try
        {
            if (Request.QueryString["XID"] != null)
            {
                ...
                dt = ClassDataAccess.GetData("Select * from TarafEstemarah Where IDUniq = @0", Convert.ToString(Request.QueryString["XID"]));
                if (dt.Rows.Count > 0)
                {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[Quaem] With(NoLock) Where AlQarabah <> @0 And IsDeleteAlQarabah = @1 Order by IDItem", string.Empty, Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    private void FGetDataMostafed()
    {
        try
        {
            ...
            if (dtGetQariah.Rows.Count > 0)
            {
                ...
                {
                    DataTable dt = new DataTable();
                    dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[RasAlEstemarah] Where NumberMostafeed = @0 And IsDelete = @1", txtSearch.Text.Trim(), Convert.ToString(false));
                    if (dt.Rows.Count > 0)
                    {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[TarafEstemarah] Where Name = @0 And NumberMostafed = @1", txtName.Text.Trim(), txtSearch.Text.Trim());
        if (dt.Rows.Count > 0)
        {
