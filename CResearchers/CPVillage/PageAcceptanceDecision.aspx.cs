...
public partial class CResearchers_CPVillage_PageAcceptanceDecision : System.Web.UI.Page
{
    string XID;
        ...
        try
        {
            foreach (GridViewRow row in GVAcceptanceDecision.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    dtGetData = ClassDataAccess.GetData("SELECT Top(10) * FROM [dbo].[QararQobolMustafeed] With(NoLock) Where IDItem = @0", Comp_ID);
    ...
    int tempcounter = 0;
    protected void GVAcceptanceDecision_RowDataBound(object sender, GridViewRowEventArgs e)
    {
