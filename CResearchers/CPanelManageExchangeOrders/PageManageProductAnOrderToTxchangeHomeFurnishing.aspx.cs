...
public partial class CResearchers_CPanelManageExchangeOrders_PageManageProductAnOrderToTxchangeHomeFurnishing : System.Web.UI.Page
{
    string XID;
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Top(1000) * FROM [dbo].[AffiliationShop] With(NoLock) Where (AffiliationName = @0 Or AffiliationName = @1 Or AffiliationName = @2 Or AffiliationName = @3 Or AffiliationName = @4) And IsActive = @5 And IsDelete = @6 Order by IDNumberAffiliation", "تاثيث منازل", "أثاث منزل", "اثاث منزل", "تأثيث منزل", "تاثيث منزل", Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[ProductShop] With(NoLock) Where IDAffiliationShop = @0 And IsActive = @1 And IsDelete = @2 Order by IDNumberProduct", DLAffiliation.SelectedValue, Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[Quaem] With(NoLock) Where (TypeAlDam = @0 Or TypeAlDam = @1 Or TypeAlDam = @2 Or TypeAlDam = @3 Or TypeAlDam = @4) And TypeAlDam <> @5 And IsDeleteTypeAlDam = @6 Order by IDItem", "تاثيث منازل", "ثاث منزل", "اثاث منزل", "تأثيث منزل", "تاثيث منزل", string.Empty, Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    protected void CBFinish_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lbmsg.Text = " (المستفيد لا يحتاج إلى تأثيث منزل) ";
                lbmsg.ForeColor = System.Drawing.Color.Red;
            }
        }
        catch (Exception)
