...
public partial class CResearchers_CPanelManageExchangeOrders_PageManageProductMatterOfExchangeForDevice : System.Web.UI.Page
{
    string XID;
    ...
    private void FGetLastRecord()
    {
        ...
        dt = ClassDataAccess.GetData("SELECT Top(1) [_billNumber] FROM [dbo].[ProductShopWarehouse] With(NoLock) Where _billNumber <> @0 And _IsDelete = @1 And _IsCart = @2 And _IsDevice = @3 And _IsTathith = @4 And _IsTalef = @5 Order by _billNumber Desc ",
            Convert.ToString(0), Convert.ToString(false), Convert.ToString(false), Convert.ToString(true), Convert.ToString(false), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Top(1000) * FROM [dbo].[CategoryShop] With(NoLock) Where (CategoryName = @0 Or CategoryName = @1 Or CategoryName = @2 Or CategoryName = @3 Or CategoryName = @4) And IsActive = @5 And IsDelete = @6 Order by IDNumberCategory", "الأجهزة الكهربائية", "أجهزة كهربائية", "الاجهزه كهربائيه", "أجهزه كهربائية", "اجهزة كهربائيه", Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[ProductShop] With(NoLock) Where IDCategoryShop = @0 And IsActive = @1 And IsDelete = @2 Order by IDNumberProduct", DLAffiliation.SelectedValue, Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[Quaem] With(NoLock) Where (TypeAlDam = @0 Or TypeAlDam = @1 Or TypeAlDam = @2 Or TypeAlDam = @3 Or TypeAlDam = @4) And TypeAlDam <> @5 And IsDeleteTypeAlDam = @6 Order by IDItem", "الاجهزة الكهربائية", "الأجهزة الكهربائية", "الاجهزه الكهربائيه", "الاجهزه الكهربائية", "الاجهزة الكهربائيه", string.Empty, Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    private void FGetDataMostafed()
    {
        ...
        {
            lbmsg.Text = "يبدو ان هذا المستفيد ليس موجود , أو خارج نطاق صلاحيتك";
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Sum([_CountProduct]) As 'Get' FROM [dbo].[ProductShopWarehouse] Where _IDProduct = @0 And _billNumber <> @1 And _IsDelete = @2", DLProduct.SelectedValue, Convert.ToString(0), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
            try
            {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Sum([_CountProduct]) As 'Set' FROM [dbo].[ProductShopWarehouse] Where _IDProduct = @0 And _billNumber = @1 And _IsDelete = @2", DLProduct.SelectedValue, Convert.ToString(0), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
            try
            {
        ...
        if (btnAdd.Text == "إضافة للفاتورة")
        {
            ...
            if (dtProduct.Rows.Count > 0)
            {
                ...
                if (Convert.ToInt64(txtCountProduct.Text.Trim()) <= Convert.ToInt64(XSumation))
                {
                    ...
                    dtRequestOfThrBeneficiary = ClassDataAccess.GetData("SELECT TOP 1 * FROM [dbo].[ReportAlZyaratElectricalAppliances] Where IDDevice = @0 And IDMustafeed = @1 And IsDelete = @2", DLProduct.SelectedValue, txtNumberMostafeed.Text.Trim(), Convert.ToString(false));
                    if (dtRequestOfThrBeneficiary.Rows.Count > 0)
                    {
                        if (Convert.ToInt64(txtCountProduct.Text.Trim()) <= Convert.ToInt64(dtRequestOfThrBeneficiary.Rows[0]["IDNumberCount"]))
                        {
                            SqlConnection conn = new SqlConnection(_Link_Connection_.ConnectionString_System_);
                            conn.Open();
                            ...
                            cmd.Parameters.AddWithValue("@IDNumberCount", Convert.ToInt64(dtRequestOfThrBeneficiary.Rows[0]["IDNumberCount"]) - Convert.ToInt64(txtCountProduct.Text.Trim()));
                            cmd.ExecuteScalar();
                            conn.Close();
                        ...
                        {
                            lbmsg.Text = "المستفيد يحتاج إلى " + dtRequestOfThrBeneficiary.Rows[0]["IDNumberCount"].ToString() + " " + DLProduct.SelectedItem.ToString() + " فقط ";
                            lbmsg.ForeColor = System.Drawing.Color.Red;
                        }
                    }
                    else
                    {
                        lbmsg.Text = "المستفيد لا يحتاج إلى " + DLProduct.SelectedItem.ToString();
                        lbmsg.ForeColor = System.Drawing.Color.Red;
                    }
                }
    ...
    private void FArnProductShopMatterOfExchangeByUser()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    ImgAmeenAlsondoq.Visible = false;
                }
                ...
                {
                    ImgAmeenAlmosTodaa.Visible = false;
                }
    ...
    private void F(int XID)
    {
        DataTable dtDevice = new DataTable();
        dtDevice = ClassDataAccess.GetData("SELECT * FROM [dbo].[ReportAlZyaratElectricalAppliances] Where IDDevice = @0 And IDMustafeed = @1 And IsDelete = @2", Convert.ToString(DLProduct.SelectedValue), txtNumberMostafeed.Text.Trim(), Convert.ToString(false));
