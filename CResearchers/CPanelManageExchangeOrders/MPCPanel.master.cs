...
public partial class CResearchers_CPanelManageExchangeOrders_MPCPanel : System.Web.UI.MasterPage
{
        ...
        if ((IDUser == IDAdmin2) && (IDUser == dtViewProfil.Rows[0]["ID_Item"].ToString()) && (IDUniq == dtViewProfil.Rows[0]["IDUniqUser"].ToString()) && (UserN == dtViewProfil.Rows[0]["User_Name_"].ToString()))
        {
            if (dtViewProfil.Rows.Count > 0)
            {
                ...
                if (A[106])
                {
                    ...
                    IDManageProductMatterOfExchangeForDamagedAdd.Visible = true;
