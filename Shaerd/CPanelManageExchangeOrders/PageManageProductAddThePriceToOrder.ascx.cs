...
public partial class Shaerd_CPanelManageExchangeOrders_PageManageProductAddThePriceToOrder : System.Web.UI.UserControl
{
    ...
    private void FArnProductShopMatterOfExchangeByUser(int IDMostafeed, float IDCategory, bool Cart, bool Device, bool Tathith, bool Talef)
    {
        try
        {
            ...
            dtGetQariah = ClassDataAccess.GetData("SELECT [_IDItem],[_billNumber],RasAlEstemarah.AlQaryah,[_IsStorekeeper][_IsCart],[_IsDevice],[_IsTathith],[_IsTalef] FROM [dbo].[ProductShopWarehouse] Inner Join RasAlEstemarah on RasAlEstemarah.NumberMostafeed = ProductShopWarehouse._IDMosTafeed Where _billNumber = @0 And _IsDelete = @1 And _IsCart = @2 And _IsDevice = @3 And _IsTathith = @4 And _IsTalef = @5"
                , txtSearch.Text.Trim(), Convert.ToString(false), Convert.ToString(RBCart.Checked), Convert.ToString(RBDevice.Checked), Convert.ToString(RBTath.Checked), Convert.ToString(RPTalef.Checked));
            if (dtGetQariah.Rows.Count > 0)
            {
                ...
                if (dtCheck.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        //Label6.Text = dt.Rows[0]["_IDMosTafeed"].ToString();
                        IDBarcode.ImageUrl = "http://chart.apis.google.com/chart?cht=qr&chl=" + ClassSetting.FGetNameServer() +
                            "/Cpanel/PScanF.aspx?ID=" + txtSearch.Text.Trim() + "&XID=" + IDMostafeed.ToString() + "&CT=" + DLSupportType.SelectedValue
                            + "&C=" + Cart.ToString() + "&D=" + Device.ToString() + "&T=" + Tathith.ToString() + "&Ta=" + Talef.ToString() + "&chs=75";
    ...
    private void FArnProductShopMatterOfExchangeByUserHouser(int billNumber, bool IsBena, bool IsTarmem)
    {
        try
        {
            GetCookie();
            DataTable dtGetQariah = new DataTable();
            dtGetQariah = ClassDataAccess.GetData("SELECT BenaaAndTarmim.IDItem,RasAlEstemarah.AlQaryah,[billNumber_],[IsTarmem],[IsBena] FROM [dbo].[BenaaAndTarmim] With(NoLock) Inner Join RasAlEstemarah on RasAlEstemarah.NumberMostafeed = BenaaAndTarmim.NumberMostafeed Where billNumber_ = @0 And IsTarmem = @1 And IsBena = @2 And BenaaAndTarmim.IsDelete = @3"
                , billNumber.ToString(), Convert.ToString(IsTarmem), Convert.ToString(IsBena), Convert.ToString(false));
            if (dtGetQariah.Rows.Count > 0)
            {
                DataTable dtCheck = new DataTable();
                dtCheck = ClassDataAccess.GetData("SELECT Top(1) [IDItem],[IDAdminJoin],[IDQariah],[IsDelete] FROM [dbo].[tbl_MultiQariah] With(NoLock) Where IDAdminJoin = @0 And IDQariah = @1 And IsDelete = @2"
    ...
    private void FArnSupportForPrismsByBillPrisms(int billNumber, float IDProject)
    {
        try
        {
            GetCookie();
            DataTable dtGetQariah = new DataTable();
            dtGetQariah = ClassDataAccess.GetData("SELECT tbl_SupportForPrisms.IDItem,RasAlEstemarah.AlQaryah,[billNumber_] FROM [dbo].[tbl_SupportForPrisms] With(NoLock) Inner Join RasAlEstemarah on RasAlEstemarah.NumberMostafeed = tbl_SupportForPrisms.NumberMostafeed Where billNumber_ = @0 And ID_Type = @1 And tbl_SupportForPrisms.IsDelete = @2"
                , billNumber.ToString(), Convert.ToString(IDProject), Convert.ToString(false));
            if (dtGetQariah.Rows.Count > 0)
            {
                DataTable dtCheck = new DataTable();
                dtCheck = ClassDataAccess.GetData("SELECT Top(1) [IDItem],[IDAdminJoin],[IDQariah],[IsDelete] FROM [dbo].[tbl_MultiQariah] With(NoLock) Where IDAdminJoin = @0 And IDQariah = @1 And IsDelete = @2"
