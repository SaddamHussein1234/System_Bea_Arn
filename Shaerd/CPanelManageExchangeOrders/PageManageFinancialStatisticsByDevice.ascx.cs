...
public partial class Shaerd_CPanelManageExchangeOrders_PageManageFinancialStatisticsByDevice : System.Web.UI.UserControl
{
    ...
    {
        ClassQuaem.FGetSupportType(1, "'2'", DLCategory);
    }
    ...
    {
        Response.Redirect("PageManageFinancialStatisticsByDevice.aspx");
    }
    ...
    public string FGetDevice(int XID)
    {
        string XResult = "";
        try
        {
            DataTable dt = new DataTable();
            dt = ClassDataAccess.GetData("SELECT DISTINCT TOP 1000 [_IDProduct],p.[ProductName] FROM [dbo].[ProductShopWarehouse] PS With(NoLock) Inner join RasAlEstemarah R on R.NumberMostafeed = PS._IDMosTafeed Inner Join ProductShop P on P.ProductID = PS._IDProduct Where R.AlQaryah = @0 And _billNumber <> @1 And _IDMosTafeed <> @2 And _IDType = @3 And _IDCategory = @4 And (convert(date, [_DateAddProduct]) Between @5 And @6) And _IsRaeesMaglisAlEdarah = @7 And _IsAmmenAlSondoq = @7 And _IsModer = @7 And _IsStorekeeper = @7 And _IsDelete = @8",
               Convert.ToString(XID), Convert.ToString(0), Convert.ToString(0), DLType.SelectedValue, DLCategory.SelectedValue, Convert.ToDateTime(txtDateFrom.Text.Trim()).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtDateTo.Text.Trim()).ToString("yyyy/MM/dd"), Convert.ToString(true), Convert.ToString(false));
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    XResult += "( " + dt.Rows[i]["ProductName"].ToString() + " : " + FGetSumDevice(XID, Convert.ToInt32(dt.Rows[i]["_IDProduct"])) + " ) ";
                }                
            }
        }
        catch (Exception)
    ...
    public string FGetSumDevice(int XID, int XIDDevice)
    {
        string XResult = "";
        try
        {
            DataTable dt = new DataTable();
            dt = ClassDataAccess.GetData("SELECT sum([_CountProduct]) As 'SumDevice' FROM [dbo].[ProductShopWarehouse] PS With(NoLock) Inner join RasAlEstemarah R on R.NumberMostafeed = PS._IDMosTafeed Where R.AlQaryah = @0 And _billNumber <> @1 And _IDMosTafeed <> @2 And  _IDProduct = @3 And _IDType = @4 And _IDCategory = @5 And (convert(date, [_DateAddProduct]) Between @6 And @7) And _IsRaeesMaglisAlEdarah = @8 And _IsAmmenAlSondoq = @8 And _IsModer = @8 And _IsStorekeeper = @8 And _IsDelete = @9",
               Convert.ToString(XID), Convert.ToString(0), Convert.ToString(0),Convert.ToString(XIDDevice), DLType.SelectedValue, DLCategory.SelectedValue, Convert.ToDateTime(txtDateFrom.Text.Trim()).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtDateTo.Text.Trim()).ToString("yyyy/MM/dd"), Convert.ToString(true), Convert.ToString(false));
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    XResult += " [ " + dt.Rows[i]["SumDevice"].ToString() + " ] جهاز";
                }
            }
        }
        catch (Exception)
    ...
    public void FGetAllDevice()
    {
        try
        {
            RPTDeviceAll.Visible = false;
            DataTable dt = new DataTable();
            dt = ClassDataAccess.GetData("SELECT DISTINCT TOP 1000 [_IDProduct],p.[ProductName] FROM [dbo].[ProductShopWarehouse] PS With(NoLock) Inner Join ProductShop P on P.ProductID = PS._IDProduct Where _billNumber <> @0 And _IDMosTafeed <> @1 And _IDType = @2 And _IDCategory = @3 And (convert(date, [_DateAddProduct]) Between @4 And @5) And _IsRaeesMaglisAlEdarah = @6 And _IsAmmenAlSondoq = @6 And _IsModer = @6 And _IsStorekeeper = @6 And _IsDelete = @7",
                Convert.ToString(0), Convert.ToString(0), DLType.SelectedValue, DLCategory.SelectedValue, Convert.ToDateTime(txtDateFrom.Text.Trim()).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtDateTo.Text.Trim()).ToString("yyyy/MM/dd"), Convert.ToString(true), Convert.ToString(false));
            if (dt.Rows.Count > 0)
            {
    ...
    public string FGetSumAllDevice(int XIDDevice)
    {
        string XResult = "";
        try
        {
