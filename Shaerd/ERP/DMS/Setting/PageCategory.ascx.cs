...
public partial class Shaerd_ERP_DMS_Setting_PageCategory : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            txt_Order_Add.Text = (Repostry_DMS_Category_.FGetCount("GetLastRecord", 1, Guid.Empty, new Guid(HFIDStore.Value), XIdentifier, string.Empty, string.Empty, string.Empty, true, "IDOrder") + 1).ToString();
            FGeAllByDate();
        }
    }
    ...
    private void FGeAllByDate()
    {
        try
        {
            ...
            dt = Repostry_DMS_Category_.FGetDataInDataTable("GeAll", 1000, Guid.Empty, new Guid(HFIDStore.Value),
             XIdentifier, txtSearch.Text.Trim(), string.Empty, string.Empty, true);
            if (dt.Rows.Count > 0)
            {
                if (Request.QueryString["ID"] == "Out_General")
                    txtTitle.Text = "قائمة فئات الصادر";
                else if (Request.QueryString["ID"] == "In_General")
                    txtTitle.Text = "قائمة فئات الوارد";
        ...
        try
        {
            ...
            foreach (GridViewRow row in GVCategory.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    Xresult = Repostry_DMS_Category_.FAPP("Delete", _XID, new Guid(HFIDStore.Value), string.Empty, string.Empty, string.Empty,
    ...
    protected void LBNew_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string Xresult = Repostry_DMS_Category_.FAPP("Add", Guid.NewGuid(), new Guid(HFIDStore.Value), XIdentifier, txtName_Ar_Add.Text.Trim(), txtName_En_Add.Text.Trim(),
    ...
    protected void LBEdit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            DataTable dt = new DataTable();
            dt = Repostry_DMS_Category_.FGetDataInDataTable("GetByIDUniq", 1, new Guid(HF_ID.Value),
    ...
    protected void LBEditData_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string Xresult = Repostry_DMS_Category_.FAPP("Edit", new Guid(HF_ID.Value), new Guid(HFIDStore.Value), XIdentifier, txtName_Ar_Edit.Text.Trim(), txtName_En_Edit.Text.Trim(),
