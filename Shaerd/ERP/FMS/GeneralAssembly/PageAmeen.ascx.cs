...
public partial class Shaerd_ERP_FMS_GeneralAssembly_PageAmeen : System.Web.UI.UserControl
{
    ...
    {
        Response.Redirect("PageAmeen.aspx");
    }
        ...
        try
        {
            foreach (GridViewRow row in GVGeneralAssemblyBill.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    cmd.Parameters.AddWithValue("@IsAllow_Raees_AlMagles", false);
                    cmd.ExecuteScalar();
                    conn.Close();
                }
            }
