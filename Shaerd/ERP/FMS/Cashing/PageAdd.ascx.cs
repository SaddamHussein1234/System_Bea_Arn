...
public partial class Shaerd_ERP_FMS_Cashing_PageAdd : System.Web.UI.UserControl
{
    ...
    private void FOM_Cashing_Add(string XType_Date)
    {
        ...
        if (Request.QueryString["ID"] == null)
        {
            Model_Cashing_ MR = new Model_Cashing_()
            {
                IDCheck = "Add",
            ...
            else if (Xresult == "IsSuccessAdd")
            {
                ...
                if (DLSend.SelectedValue == "Yes")
                {
                    ...
                    string XLong_URL = ClassSetting.FGetNameServer() + "/ar/Cashing/PageView.aspx?ID=" + txtNumberBill.Text.Trim() + "&IDUniq=" + ddlYears.SelectedValue;
                    string XResult = Attach_Repostry_Short_URL_.FShort_URL_Add("Add", XShort_URL, XLong_URL, "Om", "Cashing", XID, 0, XDate);
                    if (XResult == "IsSuccessAdd")
                        Attach_Repostry_SMS_Send_.FAddSMSMessage(HFPhoneSender.Value, "سند صرف" + "\n" + ClassSetting.FGetNameServer() + "/Check.aspx?ID=" + XShort_URL, "BerArn", "Add", XID);
                }
                if (Attach_Repostry_SMS_Send_.AllSendSystemOM())
                    Attach_Repostry_SMS_Send_.FAddSMSMessage(ClassAdmin_Arn.FGetPhoneByIDRaees(), "إضافة سند صرف" + "\n" + "رقم الفاتورة :" + txtNumberBill.Text.Trim() + "\n" + "بإنتظار الموافقة ,,,", "BerArn", "Add", XID);
        ...
        else if (Request.QueryString["ID"] != null)
        {
            ...
            else if (Xresult == "IsSuccessEdit")
            {
                ...
                if (Attach_Repostry_SMS_Send_.AllSendSystemOM())
                    Attach_Repostry_SMS_Send_.FAddSMSMessage(ClassAdmin_Arn.FGetPhoneByIDRaees(), "تعديل سند صرف" + "\n" + "رقم الفاتورة :" + txtNumberBill.Text.Trim() + "\n" + "بإنتظار الموافقة ,,,", "BerArn", "Edit", XID);
                if (Request.QueryString["ID"] != null)
                    FGetData();
            }
        }
    ...
    private void FAdd(string XIDCheck)
    {
        ...
        string Xresult = Repostry_Organstions_.FAPP_Add(XIDCheck, XID_Item, XID_Category, "_Cashing", XType_Customer,
            XName, Repostry_Organstions_.FGetCount("GetLastRecord", 1, Guid.Empty, "_Cashing", string.Empty, string.Empty, string.Empty, true, "IDOrder") + 1,
    ...
    {
        DataTable dt = new DataTable();
        dt = Repostry_Organstions_.FGetDataInDataTable("GetByIDUniq", 1, new Guid(DLCompany.SelectedValue), "_Cashing", string.Empty, string.Empty, string.Empty, true);
        if (dt.Rows.Count > 0)
        {
