...
public partial class Shaerd_ERP_FMS_Statistics_PageStatisticsReceipt : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            Repostry_Organstions_.FGetDropList(1, "_Receipt", "_ID", "_Ar", CBCompany);
    ...
    {
        Response.Redirect("PageStatisticsReceipt.aspx");
    }
                 ...
                 string XDataCheck4, string XDataCheck5, string XDataCheck6, string XFilterSearch, GridView GV)
    {
        try
        {
            ...
            Model_Receipt_ MC = new Model_Receipt_();
            MC.IDCheck = XCheck;
            ...
            MC.IsActive = true;
            DataTable dt = new DataTable();
            Repostry_Receipt_ RC = new Repostry_Receipt_();
            dt = RC.BOM_Receipt_Manage(MC);

            if (dt.Rows.Count > 0)
            {
                txtTitle.Text = " الإحصاء المالي لسندات القبض , من تاريخ " + txtDateFrom.Text.Trim() + " إلى تاريخ " + txtDateTo.Text.Trim();
