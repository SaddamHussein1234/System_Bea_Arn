...
public partial class Shaerd_CPanelManageWarehouse_PageManageCategory : System.Web.UI.UserControl
{
    ...
    private void FGetLastRecord()
    {
        ...
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[CategoryShop] With(NoLock) Where IsDelete = @0 Order by IDNumberCategory Desc", Convert.ToString(false));
        if (dt.Rows.Count > 0)
    ...
    private void FGetData()
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = ClassDataAccess.GetData("Select * from CategoryShop Where IDUniq = @0", Convert.ToString(Request.QueryString["ID"]));
            if (dt.Rows.Count > 0)
            {
    ...
    private void FGetCategoryAll()
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[CategoryShop] Where CategoryName Like '%' + @0 + '%' And IsDelete = @1 Order by IDNumberCategory", txtSearch.Text.Trim(), Convert.ToString(false));
            if (dt.Rows.Count > 0)
            {
    ...
    {
        Response.Redirect("PageManageCategory.aspx");
    }
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("Select * from CategoryShop Where IDNumberCategory = @0 And IsDelete = @1", txtNumber.Text.Trim(), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("Select * from CategoryShop Where IDStore = @0 And CategoryName = @1 And IsDelete = @2", DLStore.SelectedValue, txtName.Text.Trim(), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    private void FQariahAdd()
    {
        ...
        ClassCategory CC = new ClassCategory()
        {
            ...
            _DateAddCategory = ClassDataAccess.GetCurrentTime().ToString("yyyy/MM/dd"),
    ...
    public string FGetCount(float ID)
    {
        string XResult = "";
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Count(*) As 'CountProductByCategory' FROM [dbo].[ProductShop] Where IDCategoryShop = @0 And IsDelete = @1", Convert.ToString(ID), Convert.ToString(false));
        if (dt.Rows.Count > 0)
