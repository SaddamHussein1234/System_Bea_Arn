...
public partial class Cpanel_PageTecisionToExcludeAdd : System.Web.UI.Page
{
    string XID;
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Top(100) * FROM [dbo].[QararQobolMustafeed] With(NoLock) Where NumberQarar = @0 And IsQobol = @1 And IsEstepaad = @2 And IsDelete = @3", txtNumberQarar.Text.Trim(), Convert.ToString(false), Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Top(100) * FROM [dbo].[QararQobolMustafeed] With(NoLock) Where NumberReport = @0 And IsQobol = @1 And IsEstepaad = @2 And IsDelete = @3", txtNumberReport.Text.Trim(), Convert.ToString(false), Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
