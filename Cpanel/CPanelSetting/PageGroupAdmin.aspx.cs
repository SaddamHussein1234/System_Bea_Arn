...
public partial class Cpanel_CPanelSetting_PageGroupAdmin : System.Web.UI.Page
{
    ...
    private void FGetGroup()
    {
        ...
        dt = ClassDataAccess.GetData("SELECT Top(100) * FROM [dbo].[tbl_Group_Admin] With(NoLock) Where [Name_Group] Like '%' + @0 + '%' And [Is_Delete_Group] = @1 Order by [ID_Group] Desc", txtSearch.Text.Trim(), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
        ...
        try
        {
            foreach (GridViewRow row in GVGroupArn.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    conn.Open();
                    string sql = "UPDATE [dbo].[tbl_Group_Admin] SET [Is_Active_Group] = @IsActiveGroup WHERE [ID_Group] = @IDGroup";
