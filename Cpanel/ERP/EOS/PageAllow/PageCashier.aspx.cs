...
public partial class Cpanel_ERP_EOS_PageManage_PageCashier : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            CLS_Permissions.CheckAccountAdmin("A106", "A109", Button1, GVApprovalOfTheDirector, 0, 8);
    ...
    {
        lblCountCard.Text = WSM_Repostry_Exchange_Order_Bill_.FGetByCount_OtherStaticGeneral("Count_Cart_By_Order_Bill_By_Cacher", new Guid(ddlYears.SelectedValue), 0, string.Empty, string.Empty, "0").ToString();
        ...
        if (dt.Rows.Count > 0)
        {
            txtTitleHose.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى موافقة المشرف المالي ( لدعم بناء المنازل - ترميم المنازل ) ";
    ...
    private void FArnProductShopByModer(bool Cart, bool Device, bool Tathith, bool Talef)
    {
        try
        {
            ...
            MEOB.IDCheck = "GetByCacherWithShring";
            ...
            if (dt.Rows.Count > 0)
            {
                txtTitle.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى موافقة المشرف المالي ( الدعم العيني ) ";
    ...
    private void FArnSupportForPrismsByModer()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            txtTitlePrisms.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى موافقة المشرف المالي ( الدعم المالي ) ";
    ...
    private void FAllowModer(Guid XID)
    {
        WSM_Model_Exchange_Order_Bill_ MEOB = new WSM_Model_Exchange_Order_Bill_()
        {
            ...
            ID_Ammen_AlSondoq = 0,
            Is_Ammen_AlSondoq = true,
            Date_Ammen_AlSondoq_Allow = ClassSaddam.GetCurrentTime().ToString("yyyy-MM-dd HH:mm:ss"),
            ID_Ammen_AlSondoq_Allow = Test_Saddam.FGetIDUsiq(),
            ID_Moder = 0,
            Is_Moder = false,
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirectorHose.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    FAllowHose(Convert.ToInt32(BillID.Text), true, txtAllowStateDetalis.Text.Trim(), false, string.Empty);
                }
            }
            System.Threading.Thread.Sleep(100);
            FArnProductShopByModer(RBCardCheck.Checked, RBDeviceCheck.Checked, RBTathithCheck.Checked, RBTalefCheck.Checked);
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirectorHose.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    FAllowHose(Convert.ToInt32(BillID.Text), false, string.Empty, true, txtNotAllowStateDetalis.Text.Trim());
                }
            }
            System.Threading.Thread.Sleep(100);
            FArnProductShopByModer(RBCardCheck.Checked, RBDeviceCheck.Checked, RBTathithCheck.Checked, RBTalefCheck.Checked);
    ...
    private void FAllowHose(int XIDBill,bool XAllow, string XNoteAllow, bool XNotAllow, string XNotNoteAllow)
    {
        Model_BenaaAndTarmim_ MBAT = new Model_BenaaAndTarmim_()
        {
            IDCheck = "ByCashier",
            ...
            IDAmeenAlsondoq = 0,
            AllowState = XAllow,
            AllowStateDetalis = XNoteAllow,
            NotAllowState = XNotAllow,
            WhayNotAllow = XNotNoteAllow,
            ID_Allow_Ameen = Test_Saddam.FGetIDUsiq(),
            Date_AllowOrNotAllow = ClassSaddam.GetCurrentTime().ToString("yyyy-MM-dd HH:mm:ss"),
            IDRaeesAlMagles = 0,
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirectorPrisms.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    FAllowPrisms(Convert.ToInt32(BillID.Text), true, txtAllowStateDetalis.Text.Trim(), false, string.Empty);
                }
            }
            System.Threading.Thread.Sleep(100);
            lblCountCard.Text = WSM_Repostry_Exchange_Order_Bill_.FGetByCount_OtherStaticGeneral("Count_Cart_By_Order_Bill_By_Cacher", new Guid(ddlYears.SelectedValue), 0, string.Empty, string.Empty, "0").ToString();
    ...
    protected void btnNotAllowPrisms_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirectorPrisms.Rows)
            {
                ...
                {
                    Label BillID = (Label)row.FindControl("lblIDPrisms");
                    FAllowPrisms(Convert.ToInt32(BillID.Text), false, string.Empty, true, txtNotAllowStateDetalis.Text.Trim());
                }
            }
            System.Threading.Thread.Sleep(100);
            lblCountCard.Text = WSM_Repostry_Exchange_Order_Bill_.FGetByCount_OtherStaticGeneral("Count_Cart_By_Order_Bill_By_Cacher", new Guid(ddlYears.SelectedValue), 0, string.Empty, string.Empty, "0").ToString();
    ...
    private void FAllowPrisms(int XIDBill, bool XAllow, string XNoteAllow, bool XNotAllow, string XNotNoteAllow)
    {
        Model_SupportForPrisms_ MSFP = new Model_SupportForPrisms_()
        {
            IDCheck = "ByCashier",
