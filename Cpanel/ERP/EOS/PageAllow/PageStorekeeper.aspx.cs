...
public partial class Cpanel_ERP_EOS_PageAllow_PageStorekeeper : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            CLS_Permissions.CheckAccountAdmin("A106", "A107", Button1, GVApprovalOfTheDirector, 0, 8);
            Button1.Visible = false;
            Repostry_FinancialYear_.FErp_FinancialYear_Manage(ddlYears);
            lblCountCard.Text = WSM_Repostry_Exchange_Order_Bill_.FGetByCount_OtherStaticGeneral("Count_Cart_By_Order_Bill_By_ByStorekeeper", new Guid(ddlYears.SelectedValue), 0, string.Empty, string.Empty, "0").ToString();
    ...
    private void FArnProductShopByModer(bool Cart, bool Device, bool Tathith, bool Talef)
    {
        try
        {
            ...
            MEOB.IDCheck = "GetByStorekeeperWithShring";
            ...
            if (dt.Rows.Count > 0)
            {
                txtTitle.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى مصادقة أمين المستودع ( الدعم العيني ) ";
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirector.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    FAllowModer(new Guid(BillID.Text.ToString()), true, false);
                }
            }
    ...
    protected void btnNotAllow_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirector.Rows)
            {
                ...
                {
                    Label BillID = (Label)row.FindControl("lblID");
                    FAllowModer(new Guid(BillID.Text.ToString()), false, true);
                }
            }
            System.Threading.Thread.Sleep(100);
            lblCountCard.Text = WSM_Repostry_Exchange_Order_Bill_.FGetByCount_OtherStaticGeneral("Count_Cart_By_Order_Bill_By_ByStorekeeper", new Guid(ddlYears.SelectedValue), 0, string.Empty, string.Empty, "0").ToString();
    ...
    private void FAllowModer(Guid XID, bool _Is_Done,bool _Is_Not_Done)
    {
        WSM_Model_Exchange_Order_Bill_ MEOB = new WSM_Model_Exchange_Order_Bill_()
        {
            ...
            ID_Storekeeper = 0,
            Is_Storekeeper = true,
            Date_Storekeeper_Allow = ClassSaddam.GetCurrentTime().ToString("yyyy-MM-dd HH:mm:ss"),
            ID_Storekeeper_Allow = Test_Saddam.FGetIDUsiq(),
            Note = string.Empty,
            Is_Done = _Is_Done,
            Is_Not_Done = _Is_Not_Done,
            Is_Received = false,
