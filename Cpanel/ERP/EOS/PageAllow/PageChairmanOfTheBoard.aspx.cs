...
public partial class Cpanel_ERP_EOS_PageAllow_PageChairmanOfTheBoard : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            CLS_Permissions.CheckAccountAdmin("A106", "A110", Button1, GVApprovalOfTheDirector, 0, 8);
    ...
    {
        lblCountCard.Text = WSM_Repostry_Exchange_Order_Bill_.FGetByCount_OtherStaticGeneral("Count_Cart_By_Order_Bill_By_ByRaees", new Guid(ddlYears.SelectedValue), 0, string.Empty, string.Empty, "0").ToString();
        ...
        MBAT.IDCheck = "GetByChairmanOfTheBoard";
        ...
        if (dt.Rows.Count > 0)
        {
            txtTitleHose.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى موافقة رئيس مجلس الإدارة ( لدعم بناء المنازل - ترميم المنازل ) ";
    ...
    private void FArnProductShopByModer(bool Cart, bool Device, bool Tathith, bool Talef)
    {
        try
        {
            ...
            MEOB.IDCheck = "GetByChairmanOfTheBoardWithShring";
            ...
            if (dt.Rows.Count > 0)
            {
                txtTitle.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى موافقة رئيس مجلس الإدارة ( الدعم العيني ) ";
    ...
    private void FArnSupportForPrismsByModer()
    {
        ...
        MSFP.IDCheck = "GetByChairmanOfTheBoard";
        ...
        if (dt.Rows.Count > 0)
        {
            txtTitlePrisms.Text = "إرشيف " + ddlYears.SelectedItem.ToString() + " , قائمة أوامر الصرف التي تحتاج إلى موافقة رئيس مجلس الإدارة ( الدعم المالي ) ";
    ...
    private void FAllowModer(Guid XID)
    {
        WSM_Model_Exchange_Order_Bill_ MEOB = new WSM_Model_Exchange_Order_Bill_()
        {
            ...
            Is_Raees_Maglis_AlEdarah = true,
            Date_Raees_Allow = ClassSaddam.GetCurrentTime().ToString("yyyy-MM-dd HH:mm:ss"),
            ID_Raees_Allow = Test_Saddam.FGetIDUsiq(),
            ID_Naeb_Raees = 0,
            Is_Naeb_Raees = false,
