...
public partial class Cpanel_ERP_EOS_In_Kind_Donation_PageMatterOfExchangeForDevice : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("PageMatterOfExchangeForDevice.aspx");
    }
    ...
    private void FGetProductShopBtCategoryShop()
    {
        ...
        DLProduct.AppendDataBoundItems = true;
        DataTable dt = new DataTable();
        dt = WSM_Data_Access_Layer.GetData("SELECT Top(1000) * FROM [dbo].[ProductShop] With(NoLock) Where IDCategoryShop = @0 And IsActive = @1 And IsDelete = @2 Order by IDNumberProduct", DLCategory.SelectedValue, Convert.ToString(true), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
            DLProduct.Items.Clear();
            DLProduct.Items.Add("");
            ...
            if (Convert.ToDateTime(txt_Add.Text.Trim()).ToString("yyyy") == ddlYears.SelectedItem.ToString())
            {
                ...
                if (ClassSaddam.XCheckYear(ddlYears.SelectedItem.Text))
                {
                    ...
                    if (Convert.ToInt64(txtCountProduct.Text.Trim()) <= Convert.ToInt64(XSumation))
                    {
                        ...
                        FWSM_Model_Exchange_Order_Details_Add(XIDNumberCount);
                        //else
                        //{
    ...
    private void FWSM_Model_Exchange_Order_Details_Add(int XIDNumberCount_)
    {
        ...
        {
            //FUodateRequestOfThrBeneficiary(XIDNumberCount_);
            IDMessageWarning.Visible = false;
    ...
    private void FUodateRequestOfThrBeneficiary(int XIDNumberCount_)
    {
        ...
        if (DLSupportType.SelectedValue == "26")
        {
            ...
            cmd.Parameters.AddWithValue("@IDNumberCount", XIDNumberCount_ - Convert.ToInt64(txtCountProduct.Text.Trim()));
            cmd.ExecuteScalar();
            conn.Close();
        }
        else if (DLSupportType.SelectedValue == "64")
            ...
            foreach (GridViewRow row in GVDeedDonationInKind.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    if (Xresult == "IsSuccessDelete")
                    {
                        ...
                        string XIDCount = Convert.ToString((row.FindControl("lblCount") as Label).Text);
                        string XIDProduct = Convert.ToString((row.FindControl("lblProduct") as Label).Text);
                        if (DLSupportType.SelectedValue == "26")
                            FElectricalAppliances(Convert.ToInt32(XIDCount), XIDProduct);
                        else if (DLSupportType.SelectedValue == "64")
                            FMedicalEquipments(Convert.ToInt32(XIDCount), XIDProduct);
    ...
    private void FElectricalAppliances(int XIDCount, string XIDProduct)
    {
        DataTable dtDevice = new DataTable();
        dtDevice = ClassDataAccess.GetData("SELECT Top(1000) * FROM [dbo].[ReportAlZyaratElectricalAppliances] With(NoLock) Where IDDevice = @0 And IDMustafeed = @1 And IsDelete = @2",
            XIDProduct, txtNumberMostafeed.Text.Trim(), Convert.ToString(false));
        if (dtDevice.Rows.Count > 0)
        {
            ...
            string sql = "UPDATE [dbo].[ReportAlZyaratElectricalAppliances] SET [IDNumberCount] = @IDNumberCount Where IDDevice = @IDDevice And IDMustafeed = @IDMustafeed";
            SqlCommand cmd = new SqlCommand(sql, conn);
            cmd.Parameters.AddWithValue("@IDDevice", Convert.ToInt64(XIDProduct));
            cmd.Parameters.AddWithValue("@IDMustafeed", Convert.ToInt64(txtNumberMostafeed.Text.Trim()));
            cmd.Parameters.AddWithValue("@IDNumberCount", Convert.ToInt32(dtDevice.Rows[0]["IDNumberCount"]) + XIDCount);
            cmd.ExecuteScalar();
            conn.Close();
        }
    }

    private void FMedicalEquipments(int XIDCount, string XIDProduct)
    {
        DataTable dtDevice = new DataTable();
        dtDevice = ClassDataAccess.GetData("SELECT Top(1000) * FROM [dbo].[ReportAlZyaratMedicalEquipments] With(NoLock) Where [_ID_Device_] = @0 And [_ID_Mustafeed_] = @1 And [_IsActive_] = @2",
            XIDProduct, txtNumberMostafeed.Text.Trim(), Convert.ToString(true));
        if (dtDevice.Rows.Count > 0)
        {
            SqlConnection conn = new SqlConnection(_Link_Connection_.ConnectionString_System_);
            conn.Open();
            ...
            cmd.Parameters.AddWithValue("@IDDevice", Convert.ToInt64(XIDProduct));
            cmd.Parameters.AddWithValue("@IDMustafeed", Convert.ToInt64(txtNumberMostafeed.Text.Trim()));
            cmd.Parameters.AddWithValue("@IDNumberCount", Convert.ToInt32(dtDevice.Rows[0]["_ID_Number_Count_"]) + XIDCount);
            cmd.ExecuteScalar();
            conn.Close();
        }
    }
