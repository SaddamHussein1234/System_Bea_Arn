...
public partial class Cpanel_ERP_EOS_In_Kind_Donation_PageMatterOfExchangeHomeFurnishing : System.Web.UI.Page
{
    ...
    private void FGetData()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            ddlYears.Enabled = false; txtNumberBill.Enabled = false; txtNumberMostafeed.Enabled = false; DLSupportType.Enabled = false;
    ...
    {
        Response.Redirect("PageMatterOfExchangeHomeFurnishing.aspx");
    }
    ...
    private void FGetByBill()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                txtTitle.Text = "تقاصيل الفاتورة برقم " + MEOD.bill_Number.ToString() + ", للمستفيد " + DLName.SelectedItem.ToString();
                txtTitle.Focus();
            }
            else
            {
