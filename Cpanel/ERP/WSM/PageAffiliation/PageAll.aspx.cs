...
public partial class Cpanel_ERP_WSM_PageAffiliation_PageAll : System.Web.UI.Page
{
    ...
    private void FGetLastRecord()
    {
        ...
        dt = WSM_Data_Access_Layer.GetData("SELECT * FROM [dbo].[AffiliationShop] With(NoLock) Where IsDelete = @0 Order by IDNumberAffiliation Desc", Convert.ToString(false));
        if (dt.Rows.Count > 0)
    ...
    private void FGetData()
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = WSM_Data_Access_Layer.GetData("Select * from AffiliationShop Where IDUniq = @0", Convert.ToString(Request.QueryString["ID"]));
            if (dt.Rows.Count > 0)
            {
    ...
    private void FGetAffiliationAll()
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = WSM_Data_Access_Layer.GetData("SELECT * FROM [dbo].[AffiliationShop] Where AffiliationName Like '%' + @0 + '%' And IsDelete = @1 Order by IDNumberAffiliation", txtSearch.Text.Trim(), Convert.ToString(false));
            if (dt.Rows.Count > 0)
            {
    ...
    {
        DataTable dt = new DataTable();
        dt = WSM_Data_Access_Layer.GetData("Select * from AffiliationShop Where IDNumberAffiliation = @0 And IsDelete = @1", txtNumber.Text.Trim(), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = WSM_Data_Access_Layer.GetData("Select * from AffiliationShop Where IDStoreffiliation = @0 And AffiliationName = @1 And IsDelete = @2", DLStore.SelectedValue, txtName.Text.Trim(), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        GetCookie();
        WSM_ClassAffiliation CA = new WSM_ClassAffiliation()
        {
            ...
            _DateAddAffiliation = ClassSaddam.GetCurrentTime().ToString("yyyy/MM/dd"),
    ...
    public string FGetCount(float ID)
    {
        string XResult = "";
        DataTable dt = new DataTable();
        dt = WSM_Data_Access_Layer.GetData("SELECT Count(*) As 'CountProductByAffiliation' FROM [dbo].[ProductShop] Where IDAffiliationShop = @0 And IsDelete = @1", Convert.ToString(ID), Convert.ToString(false));
        if (dt.Rows.Count > 0)
