...
public partial class Cpanel_PageBeneficiaryAccommodationType : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            txtTitle.Text = "قائمة إحصائية المستفيدين حسب نوع المسكن ";
            //FGetNumberZero();
            FGetAlBaheth();
    ...
    private void FGetAccommodationType()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            CBLType.DataTextField = "TypeAlMaskan";
            CBLType.DataSource = dt;
            CBLType.DataBind();

            DLTypeAlmaskan.Items.Clear();
            DLTypeAlmaskan.Items.Add("");
            DLTypeAlmaskan.AppendDataBoundItems = true;
            DLTypeAlmaskan.DataValueField = "IDItem";
            DLTypeAlmaskan.DataTextField = "TypeAlMaskan";
            DLTypeAlmaskan.DataSource = dt;
            DLTypeAlmaskan.DataBind();
        }
    }
    ...
    private void FGetCharByQariah()
    {
        ...
        chartConfig.Add("caption", "إحصائية حسب مصدر نوع المسكن  " + DLAlQriah.SelectedItem.ToString());
        ...
        FusionCharts.Charts.Chart MyFirstChart = new FusionCharts.Charts.Chart("column2d", "first_chartByQariah", "100%", "340", "json", jsonDataByQariah.ToString());
        // render chart
    ...
    {
        Response.Redirect("PageBeneficiaryAccommodationType.aspx");
    }
    ...
    protected void btnGet_Click(object sender, EventArgs e)
    {
        ...
        if (DLAlQriah.SelectedItem.ToString() != string.Empty)
        {
            ...
            txtTitleByQariah.Text = " قائمة إحصائية المستفيدين حسب نوع المسكن لقرية " + DLAlQriah.SelectedItem.ToString();
            int XSum = 0;
    ...
    public string FGetCount(int XID)
    {
        string XResult = "0";
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT TOP 1000 Count(*) As 'CountTypeAlMasken' FROM [dbo].[RasAlEstemarah] With(NoLock) Where TypeAlMasken = @0 And TypeMostafeed = @1 And IsDelete = @2", Convert.ToString(XID), "دائم", Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
            XResult = dt.Rows[0]["CountTypeAlMasken"].ToString();
        }
        else
        {
            XResult = "0";
    ...
    public string FGetCountByQariah(int XID)
    {
        string XResult = "0";
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT TOP 1000 Count(*) As 'CountTypeAlMasken' FROM [dbo].[RasAlEstemarah] With(NoLock) Where AlQaryah = @0 And TypeAlMasken = @1 And TypeMostafeed = @2 And IsDelete = @3", DLAlQriah.SelectedValue, Convert.ToString(XID), "دائم", Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
            XResult = dt.Rows[0]["CountTypeAlMasken"].ToString();
        }
    ...
    private void FGetChartBySelect()
    {
        ...
        chartConfigAll.Add("caption", "إحصائية حسب نوع المسكن");
        ...
        FusionCharts.Charts.Chart MyFirstChart = new FusionCharts.Charts.Chart("column2d", "first_chart", "100%", "340", "json", jsonData.ToString());
        // render chart
    ...
    protected void btnGetByAlMasder_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            FGetMostafeedByMasderAlDhal(Convert.ToInt32(DLTypeAlmaskan.SelectedValue));
            System.Threading.Thread.Sleep(500);
        }
        catch (Exception)
        {
    ...
    private void FGetMostafeedByMasderAlDhal(int XID)
    {
        ...
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT TOP 1000 * FROM [dbo].[RasAlEstemarah] With(NoLock) inner join Quaem on Quaem.IDItem = RasAlEstemarah.AlQaryah Where TypeAlMasken = @0 And TypeMostafeed = @1 And RasAlEstemarah.IsDelete = @2 And Quaem.AlQriah <> @3 Order By AlQaryah", Convert.ToString(XID), "دائم", Convert.ToString(false), "مناطق_أخرى");
        if (dt.Rows.Count > 0)
        {
            txtSearchMostafeed.Text = "قائمة بيانات المستفيدين حسب نوع المسكن ( " + DLTypeAlmaskan.SelectedItem.ToString() + " ) ";
