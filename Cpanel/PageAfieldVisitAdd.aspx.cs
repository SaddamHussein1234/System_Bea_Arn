...
public partial class Cpanel_PageAfieldVisitAdd : System.Web.UI.Page
{
    string XID;
    ...
    protected void txtDateRegistry_TextChanged(object sender, EventArgs e)
    {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Top(1) * FROM [dbo].[ZeyarahMaydanyah] With(NoLock) Where NumberAlZyarah = @0 And IsDelete = @1", txtNumberVisit.Text.Trim(), Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    private void FZyaraAdd()
    {
        ...
        if (Attach_Repostry_SMS_Send_.AllSendSystemSocialSearch())
            Attach_Repostry_SMS_Send_.FAddSMSMessage(ClassAdmin_Arn.FGetPhoneByIDRaees(), "إضافة زيارة ميدانية" + "\n" + "رقم الملف :" + txtNumberMostafeed.Text.Trim() + "\n" + "بإنتظار الموافقة ,,,", "BerArn", "Add", Test_Saddam.FGetIDUsiq());
    }
