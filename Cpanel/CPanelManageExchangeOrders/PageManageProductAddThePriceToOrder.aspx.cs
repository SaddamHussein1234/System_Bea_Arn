...
public partial class Cpanel_CPanelManageExchangeOrders_PageManageProductAddThePriceToOrder : System.Web.UI.Page
{
                    ...
                    if (Request.QueryString["ID"] != null)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            if (Convert.ToInt32(dt.Rows[0]["_IDMosTafeed"]) != 999999999)
                            {
                                ...
                                RBDevice.Checked = Convert.ToBoolean(Request.QueryString["IsDevice"]);
                                RBTath.Checked = Convert.ToBoolean(Request.QueryString["IsTathith"]);
                                FArnProductShopMatterOfExchangeByUser(Convert.ToInt32(Request.QueryString["XID"]), Convert.ToInt64(Request.QueryString["XIDCate"])
    ...
    {
        ClassQuaem.FGetSupportType(1, "'1','2','3','6'", DLSupportType);
    }
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                string code = ClassSetting.FGetNameServer() +
                    ...
                    + IDMostafeed.ToString() + "&XIDCate=" + DLSupportType.SelectedValue + "&IsCart=" + Cart.ToString() + "&IsDevice=" + Device.ToString()
                ...
                if (dt.Rows[0]["_A1"].ToString() == string.Empty || dt.Rows[0]["_A1"].ToString() == "0")
                {
                    lblNameEvint.Text = " الإسم : " + ClassSaddam.FAlName(Convert.ToInt32(dt.Rows[0]["_IDMosTafeed"]), dt.Rows[0]["_IDType"].ToString());
                    DivNoteDevice.Visible = false;
                }
                else
                {
                    lblNameEvint.Text = dt.Rows[0]["_A1"].ToString();
                    DivNoteDevice.Visible = true;
                    lblNoteDevice.Text = ClassSaddam.FAlName(Convert.ToInt32(dt.Rows[0]["_IDMosTafeed"]), dt.Rows[0]["_IDType"].ToString());
                }
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT Top(5) [_IDItem],[_billNumber],[_IDMosTafeed] FROM [dbo].[ProductShopWarehouse] With(NoLock) Where _billNumber = @0 And [_IDCategory] = @1 And _IsDelete = @2 And _IsCart = @3 And _IsDevice = @4 And _IsTathith = @5 And _IsTalef = @6",
            txtSearch.Text.Trim(), DLSupportType.SelectedValue, Convert.ToString(false), Convert.ToString(RBCart.Checked), Convert.ToString(RBDevice.Checked), Convert.ToString(RBTath.Checked), Convert.ToString(RPTalef.Checked));
        if (dt.Rows.Count > 0)
        {
            if (Convert.ToInt32(dt.Rows[0]["_IDMosTafeed"]) != 999999999)
            {
                ...
                else
                {
                    ...
                    Session["XID"] = Convert.ToInt32(dt.Rows[0]["_IDMosTafeed"]);
                    FArnProductShopMatterOfExchangeByUser(Convert.ToInt32(dt.Rows[0]["_IDMosTafeed"]), Convert.ToInt64(DLSupportType.SelectedValue), RBCart.Checked, RBDevice.Checked, RBTath.Checked, false);
                }
            }
            else
            {
    ...
    protected void LBPrintSaraf_Click(object sender, EventArgs e)
    {
        try
        {
            GVMatterOfExchangeByID.UseAccessibleHeader = true;
            GVMatterOfExchangeByID.HeaderRow.TableSection = TableRowSection.TableHeader;
            GVMatterOfExchangeByID.Columns[0].Visible = false;
    ...
    private void F(int IDDevice, int XID)
    {
        ...
        dtDevice = ClassDataAccess.GetData("SELECT Top(1) * FROM [dbo].[ReportAlZyaratElectricalAppliances] With(NoLock) Where IDDevice = @0 And IDMustafeed = @1 And IsDelete = @2", Convert.ToString(IDDevice), txtNumberMostafeed2.Text.Trim(), Convert.ToString(false));
    ...
    private void FArnProductShopMatterOfExchangeByTaleef(bool Cart, bool Device, bool Tathith, bool Talef)
    {
        try
        {
            GVMatterOfExchangeByID.UseAccessibleHeader = false;
            IDBarcodeTalef.ImageUrl = "http://chart.apis.google.com/chart?cht=qr&chl=" + ClassSetting.FGetNameServer() +
                    "/Cpanel/PScanF.aspx?ID=" + txtSearchTalef.Text.Trim() + "&XID=" + 999999999 + "&C=" + Cart.ToString() +
                    "&D=" + Device.ToString() + "&T=" + Tathith.ToString() + "&Ta=" + Talef.ToString() + "&chs=75";
    ...
    private void FArnProductShopMatterOfExchangeByUserHouser(int billNumber, bool IsBena, bool IsTarmem)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string code = ClassSetting.FGetNameServer() + "/Cpanel/CPanelManageExchangeOrders/PageManageProductAddThePriceToOrder.aspx?IDX="
                    + txtSearchTarmim.Text.Trim() + "&XID=" + Request.QueryString["XID"]
                ...
                if (dt.Rows[0]["A2"].ToString() == string.Empty || dt.Rows[0]["A2"].ToString() == "0")
                {
                ...
                else
                {
                    ...
                    lblNameEvintTarmim.Text = dt.Rows[0]["A2"].ToString();
    ...
    private void FArnSupportForPrismsByBillPrisms(int billNumber, float IDProject)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string code = ClassSetting.FGetNameServer() + "/Cpanel/CPanelManageExchangeOrders/PageManageProductAddThePriceToOrder.aspx?IDS="
                    + billNumber.ToString() + "&IDCh=" + IDProject.ToString() + "&IDU=" + Convert.ToString(Request.QueryString["IDU"]);
                ...
                if (dt.Rows[0]["A3"].ToString() == string.Empty || dt.Rows[0]["A3"].ToString() == "0")
                {
                ...
                else
                {
                    ...
                    lblNameEvintPrisms.Text = dt.Rows[0]["A3"].ToString();
                    lblNote.Text = ClassSaddam.FAlName(Convert.ToInt32(dt.Rows[0]["NumberMostafeed"]), "1");
                }

                if (dt.Rows[0]["_Name_Initiatives__"].ToString() != "بدون مبادرة")
                    lbl_Initiatives.Text = dt.Rows[0]["_Name_Initiatives__"].ToString();
                else
