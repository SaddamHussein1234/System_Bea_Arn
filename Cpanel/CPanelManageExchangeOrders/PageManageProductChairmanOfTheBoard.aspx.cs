...
public partial class Cpanel_CPanelManageExchangeOrders_PageManageProductChairmanOfTheBoard : System.Web.UI.Page
{
    string XID;
    ...
    private void CheckAccountAdmin()
    {
        ...
        if (dtViewProfil.Rows.Count > 0)
        {
            ...
            A110 = Convert.ToBoolean(dtViewProfil.Rows[0]["A110"]);
            if (A110 == false)
                Response.Redirect("PageNotAccess.aspx");
            if (A106 == false)
            {
    ...
    {
        Response.Redirect("PageManageProductChairmanOfTheBoard.aspx");
    }
        ...
        try
        {
            foreach (GridViewRow row in GVChairmanOfTheBoard.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    conn.Open();
                    string sql = "UPDATE [dbo].[ProductShopWarehouse] SET [_IsRaeesMaglisAlEdarah] = @IsRaeesMaglisAlEdarah WHERE _billNumber = @_billNumber And _IDCategory = @IDCategory And _IsCart = @IsCart And _IsDevice = @IsDevice And _IsTathith = @IsTathith And _IsTalef = @IsTalef";
        ...
        try
        {
            foreach (GridViewRow row in GVApprovalOfTheDirectorHose.Rows)
            {
                if ((row.FindControl("chkSelect") as CheckBox).Checked)
                {
                    ...
                    SqlConnection conn = new SqlConnection(_Link_Connection_.ConnectionString_System_);
                    conn.Open();
                    string sql = "UPDATE [dbo].[BenaaAndTarmim] SET [IsAllowRaeesAlMagles] = @IsAllowRaeesAlMagles WHERE IDItem = @IDItem";
                    SqlCommand cmd = new SqlCommand(sql, conn);
