...
public partial class Cpanel_CPGeneralAssembly_PageGeneralAssembly : System.Web.UI.Page
{
    ...
    private void FGetGeneral_Assmply()
    {
        try
        {
            ...
            dt = ClassDataAccess.GetData("SELECT TOP (1000) TGA.*,TA.FirstName,TA.FirstName,[Email],[PhoneNumber],[A3] FROM [dbo].[tbl_General_Assmply] TGA With(NoLock) Inner Join tbl_Admin TA on TA.ID_Item = TGA.ID_Admin_Account_ Where ([FirstName] Like '%' + @0 + '%') And [Is_Delete_] = @1 Order by [Number_Rigstry_]",
    ...
    protected void LBPrintAll_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            GVAdmin.Columns[11].Visible = false;
