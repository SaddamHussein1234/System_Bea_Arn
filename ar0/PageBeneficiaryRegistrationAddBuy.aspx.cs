...
public partial class ar_PageBeneficiaryRegistrationAddBuy : System.Web.UI.Page
{
    ...
    private void FGetData()
    {
        ...
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[TarafEstemarah] Where NumberMostafed = @0 And IsDelete = @1", lblFile.Text, Convert.ToString(false));
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = new DataTable();
        dt = ClassDataAccess.GetData("SELECT * FROM [dbo].[TarafEstemarah] Where Name = @0 And NumberMostafed = @1", txtName.Text.Trim(), lblFile.Text);
        if (dt.Rows.Count > 0)
        {
